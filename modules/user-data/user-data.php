...
class Inzite_User_Data
{
	...
	{
		return '<div class="login_message"><p>Køb adgang her hvis du ikke <br/>allerede har det. <br><a href="' . get_term_link(9, 'product_cat') . '" class="button button-primary">Køb adgang!</a></p></div>';
	}
	...
	function inzite_profile_menu()
	{
		...
		if ($current_user->ID != 0) {
			...
			echo '<li><a href="#" id="chat-toggle">Chat';
	...
	function inzite_profile_header($current_user = '', $user_data = '')
	{
		...
		if ($current_user->ID != 0) {
			...
			echo '</div>';
			echo '<h1>';
			echo '<a href="' . site_url('/profile/' . $user_data->data->user_nicename . '/') . '">' . $user_data->user_firstname . ' ' . $user_data->user_lastname . '</a>';
			echo '</h1>';
	...
	function inzite_profile_users($current_user, $user_is_parent = 0)
	{
		...
		foreach ($subusers as $key => $subuser) {
			...
			echo '<a href="?deleteuser=true&userid=' . $subuser->user_id . '">Slet</a>';
			echo '</td>';
			echo '<td>';
			echo '</tr>';
		}
	...
	function inzite_profile_form($user_data = array(), $current_user, $user_is_parent = 0)
	{
		if ($user_data) {
			...
			echo '<label for="user_login">Brugernavn (kan desværre ikke ændres.)</label>';
			echo '<input class="text-input" name="user_login" type="text" readonly="readonly" value="' . $user_data->data->user_login . '" />';
			echo '</p>';

			echo '<p class="form-display_name" style="display: none;">';
			...
			if (count($subusers) < $user_is_parent) {
				...
				echo '<p class="form-user_pass">';
				echo '<label for="user_pass">Adgangskode</label>';
				echo '<input class="text-input" name="user_pass" type="password" value="" />';
				echo '</p>';

				echo '<p class="form-user_pass2">';
				echo '<label for="user_pass2">Adgangskode (Gentag)</label>';
				echo '<input class="text-input" name="user_pass2" type="password" value="" />';
				echo '</p>';
